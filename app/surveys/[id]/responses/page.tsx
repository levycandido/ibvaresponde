  ...
  const responsesByQuestion = respostas.reduce(
    ...
    {} as Record<
      string,
      {
        ...
        respostas: typeof respostas
      }
    >
  )

  // Count responses by answer
  const countResponsesByAnswer = (respostas: typeof respostas) => {
      ...
      <Container withBottomNav>
        <div className="relative">
          ...
          <motion.div
            ...
            className="px-4 sm:px-0 pt-4 pb-8"
          >
          ...
          >
            {Object.entries(responsesByQuestion).map(([questionId, data], questionIndex) => {
                ...
                >
                  ...
                  <div className="px-6 py-6">
                    <div className="space-y-3">
                      {data.respostas.map((resp, respIndex) => (
                        <motion.div
                          key={resp.surveyResponseId}
                          initial={{ opacity: 0, x: -10 }}
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: respIndex * 0.05 }}
                          className="flex items-start justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100 hover:border-blue-300 transition-colors"
                        >
                          <div className="flex-1">
